...
## 🚀 Phase 7: 个人使用成本优化 🎯 规划中

### 🎯 P0 - 免费资源最大化利用 (Free Resource Maximization)
**目标**: 将免费API额度利用率最大化，避免不必要的付费消耗

- [ ] **免费优先路由策略**
  - 新增 `free_first` 路由策略，优先使用所有免费渠道
  - 只有免费渠道全部不可用或超额时才使用付费渠道
  - 配置示例: `default_strategy: "free_first"`
...
### 🎯 P0.5 - 本地模型优先策略 (Local Model Priority)
**目标**: 最大化利用本地计算资源，减少云端API调用

- [ ] **本地优先模式**
  - 新增 `local_first` 模式，优先使用本地Ollama/LMStudio
  - 除非本地模型无法处理特定需求才使用云端
  - 配置项: `prefer_local: true`
...
### 🎯 P1.5 - 使用体验优化 (User Experience Enhancement)
**目标**: 在保证成本控制的前提下提升使用体验

- [ ] **成本透明化**
  - 响应头显示本次请求实际成本
  - 累计成本统计和节省金额显示
  - 成本对比功能(显示使用其他渠道的成本差异)
