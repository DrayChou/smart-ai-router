...
def create_usage_stats_router(config_loader: YAMLConfigLoader) -> APIRouter:
    ...
    async def get_daily_stats(
        target_date: Optional[str] = Query(None, description="目标日期 (YYYY-MM-DD格式)，默认为今天"),
    ...
    async def get_weekly_stats(
        target_date: Optional[str] = Query(None, description="目标日期 (YYYY-MM-DD格式)，默认为今天所在周"),
        auth: bool = Depends(get_admin_auth_dependency)
    ):
        """获取本周使用统计"""
        try:
    ...
    async def get_usage_summary(
        ...
        try:
            ...
            if weekly_stats['total_requests'] > 0:
                weekly_avg = weekly_stats['total_cost'] / max(1, (today.weekday() + 1))  # 按已过天数计算
                if weekly_avg > 0:
                    summary["comparison"]["daily_vs_weekly_avg"] = (daily_stats['total_cost'] - weekly_avg) / weekly_avg
            
            if monthly_stats['total_requests'] > 0:
                monthly_avg = monthly_stats['total_cost'] / max(1, today.day)  # 按已过天数计算
                weekly_avg_monthly = weekly_stats['total_cost'] / 7 * 7  # 本周总消费
                if monthly_avg > 0:
                    summary["comparison"]["weekly_vs_monthly_avg"] = (weekly_avg_monthly - monthly_avg * 7) / (monthly_avg * 7)
    ...
    async def get_top_models(
        ...
        target_date: Optional[str] = Query(None, description="目标日期 (YYYY-MM-DD格式)"),
        auth: bool = Depends(get_admin_auth_dependency)
    ):
        """获取使用最多的模型排行"""
        try:
            ...
            for model, info in sorted_models:
                result.append({
                    ...
                    "avg_tokens_per_request": info['tokens'] / max(1, info['requests'])
                })
    ...
    async def get_cost_breakdown(
        period: str = Query("daily", regex="^(daily|weekly|monthly)$", description="统计周期"),
        breakdown_by: str = Query("provider", regex="^(provider|channel|model)$", description="分解维度"),
        target_date: Optional[str] = Query(None, description="目标日期 (YYYY-MM-DD格式)"),
        auth: bool = Depends(get_admin_auth_dependency)
    ):
        """获取成本分解分析"""
