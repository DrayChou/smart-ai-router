...
def create_token_estimation_router(config_loader) -> APIRouter:
    ...
    async def estimate_tokens(request: TokenEstimationRequest):
        ...
        try:
            ...
            # 执行Token预估
            token_estimate = token_estimator.estimate_tokens(
                messages, request.model_family
            )
            ...
            recommendations = model_optimizer.recommend_models(
                token_estimate, available_channels, request.optimization_strategy
            )
    ...
    async def calculate_model_pricing(request: ModelPricingRequest):
        ...
        try:
            ...
            for channel in all_channels:
                if (
                    channel.model_name.lower() == request.model_name.lower()
                    and channel.provider.lower() == request.provider.lower()
                ):
        ...
        try:
            ...
            for channel in all_channels:
                ...
                if model_name not in model_scores:
                    ...
                    speed_score = model_optimizer.get_model_speed_score(
                        model_name, channel.provider
                    )
            ...
            return {"total_models": len(sorted_models), "models": sorted_models}
