...
class ChatGPTCompletionRequest(BaseModel):
    ...
    top_logprobs: Optional[int] = Field(
        None, description="返回的top log概率数量", ge=0, le=20
    )
...
def convert_to_chat_request(
    request: ChatGPTCompletionRequest, api_key: Optional[str]
    ...
    if request.tools:
        ...
        for tool in request.tools:
            tools.append({"type": tool.type, "function": tool.function})
...
async def stream_chatgpt_response(
    ...
    original_request: ChatGPTCompletionRequest,
):
    """流式生成OpenAI格式响应"""
