...
from core.utils.model_channel_blacklist import get_model_blacklist_manager, ErrorType
...
async def manage_blacklist_entry(
    request: BlacklistManagementRequest,
    admin_token: str = Depends(verify_admin_token)
):
    """管理黑名单条目（移除、延长、设为永久）"""
    blacklist_manager = get_model_blacklist_manager()
    ...
    if request.action == "remove":
        ...
        if success:
            return {"message": f"Successfully removed {request.model_name}@{request.channel_id} from blacklist"}
        else:
            raise HTTPException(
                status_code=404, 
        ...
        if not request.duration:
            raise HTTPException(status_code=400, detail="Duration required for extend action")
        ...
        if key in blacklist_manager._blacklist:
            ...
            if not entry.is_permanent:
                ...
                return {"message": f"Extended blacklist for {request.model_name}@{request.channel_id} by {request.duration} seconds"}
            else:
                raise HTTPException(status_code=400, detail="Cannot extend permanent blacklist")
        else:
        ...
        if key in blacklist_manager._blacklist:
            ...
            return {"message": f"Set {request.model_name}@{request.channel_id} as permanently blacklisted"}
        else:
            raise HTTPException(status_code=404, detail="Blacklist entry not found")
...
async def get_health_report(admin_token: str = Depends(verify_admin_token)):
    ...
    for channel in all_channels:
        ...
        # 获取该渠道的黑名单信息
        blacklisted_model_names = blacklist_manager.get_blacklisted_models_for_channel(channel.id)
...
async def get_channel_model_status(
    ...
    # 获取该渠道的黑名单模型
    blacklisted_models = blacklist_manager.get_blacklisted_models_for_channel(channel_id)
    
    # 构建响应
    model_status = []
    for model_name in blacklisted_models:
        is_blacklisted, entry = blacklist_manager.is_model_blacklisted(channel_id, model_name)
