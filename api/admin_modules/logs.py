...
async def get_slow_requests(
    ...
    try:
        slow_requests = await analyzer.get_slow_requests(
            min_duration=min_duration, hours=hours, limit=limit
        )
...
async def get_request_timeline(
    ...
    try:
        ...
        if not timeline:
            raise HTTPException(
                status_code=404, detail=f"未找到请求ID {request_id} 的日志"
            )
...
async def get_logging_health(
    ...
    try:
        ...
        if logs_dir.exists():
            total_size = sum(
                f.stat().st_size for f in logs_dir.glob("*") if f.is_file()
            )
            backup_count = len(list(logs_dir.glob("*.log.*")))

            health_info.update(
                {"logs_directory_size": total_size, "backup_files_count": backup_count}
            )
