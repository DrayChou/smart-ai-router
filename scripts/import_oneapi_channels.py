...
def import_channels_from_oneapi():
    ...
    for ch in channels:
        ...
        channel_config = {
            ...
            "weight": 1,
            "daily_limit": 1000,
            "cost_per_token": {"input": 1e-6, "output": 1e-6},
        ...
        if models:
            ...
            if model_list:
                ...
                channel_config["configured_models"] = (
                    model_list  # 保存完整的模型列表作为备选
                )
        ...
        if ch_type == 31:  # Groq类型
            ...
            if "zhipu" in name.lower() or "glm" in models.lower():
                channel_config["base_url"] = (
                    base_url or "https://open.bigmodel.cn/api/paas"
                )
            elif "doubao" in name.lower():
                channel_config["base_url"] = (
                    base_url or "https://ark.cn-beijing.volces.com/api"
                )
        ...
        if any(
            ...
            for provider in ["groq", "openrouter", "burn", "moonshot"]
        ):
            tags.append("cloud")
        ...
        print(
            f"[OK] 导入渠道: {name} (ID: {ch_id}, 优先级: {priority}, 模型数: {len(models.split(',')) if models else 0})"
        )
