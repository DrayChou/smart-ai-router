...
class PerformanceTest:
    ...
    async def test_batch_performance(
        self, test_model: str = "tag:gpt", iterations: int = 5
    ):
        """测试批量评分性能"""
        ...
        for i in range(iterations):
            ...
            try:
                ...
                if results:
                    ...
                    print(
                        f"    OK Found {len(results)} channels in {duration_ms:.1f}ms"
                    )
                else:
    ...
    async def run_comprehensive_test(self):
        ...
        test_models = ["tag:gpt", "tag:free", "tag:qwen", "tag:claude"]
        ...
        print("\nPERFORMANCE SUMMARY:")
        perf_results = [
            v for k, v in all_results.items() if k.startswith("performance_")
        ]
        if perf_results:
            avg_per_channel = sum(r["avg_per_channel_ms"] for r in perf_results) / len(
                perf_results
            )
        ...
        if cache_results:
            avg_speedup = sum(
                r["speedup"] for r in cache_results if r["speedup"] > 0
            ) / len([r for r in cache_results if r["speedup"] > 0])
    ...
    async def quick_test(self, model: str = "tag:free"):
        ...
        if results:
            ...
            print(
                f"OK Found {len(results)} channels in {duration:.1f}ms ({per_channel:.1f}ms per channel)"
            )
            ...
            for i, result in enumerate(results[:3], 1):
                print(
                    f"  #{i}: {result.channel.name} (Score: {result.total_score:.3f})"
                )
        else:
