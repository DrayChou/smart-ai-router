...
class ProfessionalAPITester:
    ...
    def __init__(self, base_url: str = "http://localhost:7602", api_key: str = "test-key"):
    ...
    def make_request(self, method: str, url: str, headers: Dict[str, str], 
                    data: Optional[Dict] = None, stream: bool = False) -> Dict[str, Any]:
        """统一请求方法"""
        try:
            if method.upper() == "GET":
    ...
    def test_basic_root(self):
        ...
        if result.get("status_code") == 200:
            self.results["basic"]["root"] = {"status": "success", "response": result.get("data", {})}
            print("[成功] 根路径测试成功")
        else:
            self.results["basic"]["root"] = {"status": "failed", "error": result.get("error", "Unknown error")}
            print("[失败] 根路径测试失败")
    ...
    def test_basic_health(self):
        ...
        if result.get("status_code") == 200:
            self.results["basic"]["health"] = {"status": "success", "response": result.get("data", {})}
            print("[成功] 健康检查测试成功")
        else:
            self.results["basic"]["health"] = {"status": "failed", "error": result.get("error", "Unknown error")}
            print("[失败] 健康检查测试失败")
    ...
    def test_basic_models(self):
        ...
        if result.get("status_code") == 200:
            ...
            self.results["basic"]["models"] = {"status": "success", "model_count": model_count}
            print(f"[成功] 模型列表测试成功，发现 {model_count} 个模型")
        else:
            self.results["basic"]["models"] = {"status": "failed", "error": result.get("error", "Unknown error")}
            print("[失败] 模型列表测试失败")
    ...
    def test_openai_text(self):
        ...
        result = self.make_request("POST", f"{self.base_url}/v1/chat/completions", headers, data)
        
        if result.get("status_code") == 200:
            response_data = result.get("data", {})
            content = response_data.get("choices", [{}])[0].get("message", {}).get("content", "")
            self.results["openai"]["text"] = {"status": "success", "content": content[:50] + "..."}
            print(f"[成功] OpenAI文本聊天成功: {content[:50]}...")
        else:
            self.results["openai"]["text"] = {"status": "failed", "error": result.get("error", "Unknown error")}
    ...
    def test_openai_streaming(self):
        ...
        result = self.make_request("POST", f"{self.base_url}/v1/chat/completions", headers, data, stream=True)
        
        if result.get("status_code") == 200:
            chunks = result.get("chunks", [])
            content_chunks = [chunk for chunk in chunks if chunk.startswith("data: ") and "content" in chunk]
            self.results["openai"]["streaming"] = {"status": "success", "chunks": len(content_chunks)}
            print(f"[成功] OpenAI流式聊天成功，接收 {len(content_chunks)} 个内容块")
        else:
            self.results["openai"]["streaming"] = {"status": "failed", "error": result.get("error", "Unknown error")}
    ...
    def test_openai_vision(self):
        ...
        if result.get("status_code") == 200:
            response_data = result.get("data", {})
            content = response_data.get("choices", [{}])[0].get("message", {}).get("content", "")
            self.results["openai"]["vision"] = {"status": "success", "content": content[:50] + "..."}
            ...
            if "No available channels" in error_msg or "vision" in error_msg.lower():
                self.results["openai"]["vision"] = {"status": "failed", "error": "Vision capability not available in current channels"}
                print("[跳过] OpenAI多模态聊天 - 渠道不支持vision功能")
            else:
                self.results["openai"]["vision"] = {"status": "failed", "error": f"{status_code}: {error_msg}"}
    ...
    def test_openai_tools(self):
        ...
        if result.get("status_code") == 200:
            ...
            self.results["openai"]["tools"] = {"status": "success", "tool_calls": len(tool_calls)}
            print(f"[成功] OpenAI工具调用成功，检测到 {len(tool_calls)} 个工具调用")
        else:
            self.results["openai"]["tools"] = {"status": "failed", "error": result.get("error", "Unknown error")}
    ...
    def test_anthropic_text(self):
        ...
        result = self.make_request("POST", f"{self.base_url}/v1/messages", headers, data)
        
        if result.get("status_code") == 200:
            response_data = result.get("data", {})
            content = response_data.get("content", [{}])[0].get("text", "")
            self.results["anthropic"]["text"] = {"status": "success", "content": content[:50] + "..."}
            print(f"[成功] Anthropic文本聊天成功: {content[:50]}...")
        else:
            self.results["anthropic"]["text"] = {"status": "failed", "error": result.get("error", "Unknown error")}
    ...
    def test_anthropic_streaming(self):
        ...
        result = self.make_request("POST", f"{self.base_url}/v1/messages", headers, data, stream=True)
        
        if result.get("status_code") == 200:
            chunks = result.get("chunks", [])
            content_chunks = [chunk for chunk in chunks if "content_block_delta" in chunk]
            self.results["anthropic"]["streaming"] = {"status": "success", "chunks": len(content_chunks)}
            print(f"[成功] Anthropic流式聊天成功，接收 {len(content_chunks)} 个内容块")
        else:
            self.results["anthropic"]["streaming"] = {"status": "failed", "error": result.get("error", "Unknown error")}
    ...
    def test_anthropic_vision(self):
            ...
            "messages": [
                {
                    ...
                    "content": [
                        ...
                        {"type": "image", "source": {"type": "base64", "media_type": "image/png", "data": self.base64_image}}
                    ]
                }
            ],
            "max_tokens": 50
        ...
        if result.get("status_code") == 200:
            response_data = result.get("data", {})
            content = response_data.get("content", [{}])[0].get("text", "")
            self.results["anthropic"]["vision"] = {"status": "success", "content": content[:50] + "..."}
            print(f"[成功] Anthropic多模态聊天成功: {content[:50]}...")
        else:
            error_msg = result.get("error", "Unknown error")
            if "No available channels" in error_msg or "vision" in error_msg.lower():
                self.results["anthropic"]["vision"] = {"status": "failed", "error": "Vision capability not available in current channels"}
                print("[跳过] Anthropic多模态聊天 - 渠道不支持vision功能")
            else:
                self.results["anthropic"]["vision"] = {"status": "failed", "error": error_msg}
    ...
    def test_anthropic_tools(self):
        ...
        if result.get("status_code") == 200:
            ...
            self.results["anthropic"]["tools"] = {"status": "success", "has_tool_use": has_tool_use}
            ...
            self.results["anthropic"]["tools"] = {"status": "failed", "error": f"{status_code}: {error_msg}"}
    ...
    def test_gemini_text(self):
        ...
        result = self.make_request("POST", f"{self.base_url}/v1beta/models/{self.model_gemini}:generateContent", headers, data)
        ...
        if result.get("status_code") == 200:
            ...
            if candidates:
                ...
                self.results["gemini"]["text"] = {"status": "success", "content": content[:50] + "..."}
                print(f"[成功] Gemini文本聊天成功: {content[:50]}...")
            else:
                self.results["gemini"]["text"] = {"status": "failed", "error": "No candidates in response"}
                print("[失败] Gemini文本聊天失败: 无候选结果")
        else:
            self.results["gemini"]["text"] = {"status": "failed", "error": result.get("error", "Unknown error")}
    ...
    def test_gemini_streaming(self):
        ...
        result = self.make_request("POST", f"{self.base_url}/v1beta/models/{self.model_gemini}:streamGenerateContent", headers, data, stream=True)
        
        if result.get("status_code") == 200:
            chunks = result.get("chunks", [])
            content_chunks = [chunk for chunk in chunks if "candidates" in chunk]
            self.results["gemini"]["streaming"] = {"status": "success", "chunks": len(content_chunks)}
            print(f"[成功] Gemini流式聊天成功，接收 {len(content_chunks)} 个内容块")
        else:
            self.results["gemini"]["streaming"] = {"status": "failed", "error": result.get("error", "Unknown error")}
    ...
    def test_gemini_vision(self):
        ...
        data = {
            "contents": [{
                "parts": [
                    ...
                    {"inline_data": {"mime_type": "image/png", "data": self.base64_image}}
                ]
            }]
        }
        ...
        if result.get("status_code") == 200:
            ...
            if candidates:
                content = candidates[0].get("content", {}).get("parts", [{}])[0].get("text", "")
                self.results["gemini"]["vision"] = {"status": "success", "content": content[:50] + "..."}
                print(f"[成功] Gemini多模态聊天成功: {content[:50]}...")
            else:
                self.results["gemini"]["vision"] = {"status": "failed", "error": "No candidates in response"}
            ...
            if "No available channels" in error_msg or "vision" in error_msg.lower():
                self.results["gemini"]["vision"] = {"status": "failed", "error": "Vision capability not available in current channels"}
                print("[跳过] Gemini多模态聊天 - 渠道不支持vision功能")
            else:
                self.results["gemini"]["vision"] = {"status": "failed", "error": f"{status_code}: {error_msg}"}
    ...
    def test_gemini_tools(self):
        ...
        result = self.make_request("POST", f"{self.base_url}/v1beta/models/{self.model_tools}:generateContent", headers, data)
        ...
        if result.get("status_code") == 200:
            ...
            if candidates:
                ...
                self.results["gemini"]["tools"] = {"status": "success", "tool_called": tool_called, "finish_reason": finish_reason}
                print(f"[成功] Gemini工具调用成功，工具调用检测: {tool_called} (原因: {finish_reason})")
            else:
                self.results["gemini"]["tools"] = {"status": "failed", "error": "No candidates in response"}
                print("[失败] Gemini工具调用失败: 无候选结果")
        else:
            self.results["gemini"]["tools"] = {"status": "failed", "error": result.get("error", "Unknown error")}
