    ...
    async def benchmark_common_scenarios(self):
        ...
        for model_query, description in test_cases:
            ...
            try:
                ...
                if channels:
                    print(f"  Cold cache: {cold_time:.1f}ms -> {len(channels)} channels")
                else:
        ...
        if successful_tests:
            avg_cold = sum(r['cold_time_ms'] for r in successful_tests) / len(successful_tests)
            avg_hot = sum(r['hot_time_ms'] for r in successful_tests) / len(successful_tests)
            avg_speedup = sum(r['speedup'] for r in successful_tests if r['speedup'] > 0) / len([r for r in successful_tests if r['speedup'] > 0])
