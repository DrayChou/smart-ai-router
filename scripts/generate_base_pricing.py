...
from core.pricing.unified_format import UnifiedModelData, UnifiedPricingFile
...
def generate_base_pricing(openrouter_unified):
    """从OpenRouter数据生成基础定价"""
    print(
        f"基于OpenRouter数据生成基础定价文件，共{len(openrouter_unified.models)}个模型"
    )
    ...
    for model_id, model_data in openrouter_unified.models.items():
        ...
        if model_data.pricing:
            if model_data.pricing.prompt == 0 and model_data.pricing.completion == 0:
                ...
                # 价格范围统计 (转换为每百万tokens)
                avg_price = (
                    (model_data.pricing.prompt + model_data.pricing.completion)
                    / 2
                    * 1_000_000
                )
                if avg_price < 0.001:
...
def print_base_pricing_stats(stats):
    ...
    print(
        f"  视觉模型: {stats['vision_models']:>3} 个 ({stats['vision_models']/total*100:>4.1f}%)"
    )
    print(
        f"  推理模型: {stats['reasoning_models']:>3} 个 ({stats['reasoning_models']/total*100:>4.1f}%)"
    )
    print(
        f"  代码模型: {stats['code_models']:>3} 个 ({stats['code_models']/total*100:>4.1f}%)"
    )
    print(
...
def main():
    ...
    # 保存基础定价文件
    base_pricing_path = (
        project_root / "config" / "pricing" / "base_pricing_unified.json"
    )
