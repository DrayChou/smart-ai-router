...
class DiagnosticTool:
    ...
    def add_issue(self, category: str, title: str, description: str, solution: str = ""):
        """添加问题"""
    ...
    def add_warning(self, category: str, title: str, description: str, suggestion: str = ""):
        """添加警告"""
    ...
    def check_configuration(self):
        ...
        # 检查必需的配置项
        required_sections = ['system', 'server', 'providers', 'channels', 'routing', 'tasks']
    ...
    def check_network_connectivity(self):
        ...
        success, _, _ = self.run_command(
            "curl -s --max-time 10 -I https://api.openai.com/v1/models",
            timeout=15
        )
        if success:
        ...
        if self.check_file_exists(cache_dir):
            ...
            for filename, description in key_cache_files:
                ...
                if self.check_file_exists(cache_file):
                    try:
                        with open(self.project_root / cache_file, 'r', encoding='utf-8') as f:
                            data = json.load(f)
                        self.add_info("cache", description, f"缓存有效，包含 {len(data)} 个条目")
                    except Exception as e:
                        self.add_warning(
                            "cache",
                            f"{description}损坏",
                    ...
                    self.add_info("cache", description, "缓存文件不存在（正常，首次运行会生成）")
        else:
    ...
    def check_port_availability(self):
        ...
        success, stdout, _ = self.run_command("ps aux | grep 'smart-ai-router\\|main.py' | grep -v grep")
        if success and stdout.strip():
...
def main():
    ...
    parser.add_argument(
        "--category",
        choices=["environment", "configuration", "dependencies", "network", "service", "cache", "port"],
        help="只检查特定类别"
    )
    parser.add_argument(
        "--output",
