...
class LogAnalyzer:
    ...
    async def get_log_stats(
        ...
        async for entry in self._read_log_entries():
            ...
            if level in ['ERROR', 'CRITICAL']:
                ...
                pattern = re.sub(r'\b[a-f0-9-]{8,}\b', '<ID>', pattern, flags=re.IGNORECASE)
    ...
    async def get_request_timeline(
        ...
        # 按时间戳排序
        entries.sort(key=lambda x: self._parse_timestamp(x.get('timestamp')) or datetime.min)
    ...
    def _matches_query(self, entry: Dict[str, Any], query: LogQuery) -> bool:
        ...
        if query.start_time or query.end_time:
            # 尝试多个时间戳字段
            entry_time = self._parse_timestamp(entry.get('timestamp') or entry.get('asctime'))
            if not entry_time:
                return False
...
class LogDashboard:
    ...
    async def check_alerts(self) -> List[Dict[str, Any]]:
        ...
        error_count = stats.level_counts.get('ERROR', 0) + stats.level_counts.get('CRITICAL', 0)
