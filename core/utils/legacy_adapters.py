    ...
    def analyze_model(
        ...
        if metadata:
            return ModelSpecs(
                ...
                parameter_size_text=self._format_parameter_size(
...
class LocalModelCapabilitiesAdapter:
    ...
    async def detect_model_capabilities(
        ...
        if (
            ...
            and not self._is_local_provider(provider, base_url)
        ):
            # 云端模型直接返回统一注册表数据
    ...
    def _get_basic_cloud_capabilities(
        self, model_name: str, provider: str, base_url: str
    ) -> LegacyModelCapabilities:
        """获取基础云端能力"""
        model_lower = model_name.lower()
        provider_lower = provider.lower()
        ...
        if provider_lower in ["openai", "anthropic", "google"]:
            if any(
                keyword in model_lower for keyword in ["gpt-4", "claude-3", "gemini"]
            ):
