...
class SessionManager:
    ...
    def create_user_identifier(self, api_key: Optional[str], user_agent: Optional[str], 
    ...
    def get_or_create_session(self, user_identifier: str) -> UserSession:
        ...
        if user_identifier in self.sessions:
            ...
            if not session.is_expired(self.session_timeout):
                ...
                logger.info(f"🧹 SESSION EXPIRED: {user_identifier} (requests: {session.total_requests}, cost: {session.get_formatted_cost()})")
    ...
    def add_request(self, user_identifier: str, cost: float, model: str, channel: str) -> UserSession:
        """添加请求到会话"""
        session = self.get_or_create_session(user_identifier)
        session.add_request(cost, model, channel)
        
        logger.debug(f"📊 SESSION UPDATE: {user_identifier} - requests: {session.total_requests}, cost: {session.get_formatted_cost()}")
    ...
    def _maybe_cleanup(self):
        ...
        if (now - self._last_cleanup) > self.cleanup_interval:
            ...
            for user_id in expired_sessions:
                ...
                logger.info(f"🧹 CLEANUP SESSION: {user_id} (requests: {session.total_requests}, cost: {session.get_formatted_cost()})")
                del self.sessions[user_id]
            
            if expired_sessions:
                logger.info(f"🧹 CLEANUP COMPLETE: Removed {len(expired_sessions)} expired sessions")
