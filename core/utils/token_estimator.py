...
class TokenEstimator:
    ...
    def __init__(self):
        ...
        self.complexity_keywords = {
            TaskComplexity.SIMPLE: [
                'hi', 'hello', 'translate', '翻译', '你好', '简单', 'simple', 
                'what', 'how are you', '谢谢', 'thank'
            ],
            TaskComplexity.MODERATE: [
                'summarize', '总结', 'explain', '解释', 'analyze', '分析', 
                'compare', '比较', 'list', '列出', 'describe', '描述'
            ],
            TaskComplexity.COMPLEX: [
                'code', '代码', 'programming', '编程', 'algorithm', '算法',
                'design', '设计', 'implement', '实现', 'debug', '调试',
                'optimize', '优化', 'architecture', '架构'
            ],
            TaskComplexity.EXPERT: [
                'research', '研究', 'thesis', '论文', 'academic', '学术',
                'scientific', '科学', 'mathematical', '数学', 'proof', '证明',
                'complex analysis', '复杂分析', 'deep dive', '深入'
            ]
        }
    ...
    def estimate_input_tokens(self, messages: List[Dict[str, str]], model_family: str = "gpt") -> int:
        """估算输入tokens数量"""
    ...
    def estimate_output_tokens(self, input_tokens: int, complexity: TaskComplexity) -> Tuple[int, float]:
        """估算输出tokens数量"""
    ...
    def estimate_tokens(self, messages: List[Dict[str, str]], model_family: str = "gpt") -> TokenEstimate:
        """完整的token估算"""
        ...
        # 估算输出tokens
        output_tokens, confidence = self.estimate_output_tokens(input_tokens, complexity)
...
class ModelOptimizer:
    ...
    def get_model_speed_score(self, model_name: str, provider: str = "") -> float:
        ...
        # 根据模型类型估算
        if 'mini' in model_name_lower or any(size in model_name_lower for size in ['7b', '8b']):
            return 150  # 小模型更快
    ...
    def calculate_estimated_cost(self, tokens_estimate: TokenEstimate, 
        ...
        total_cost = (tokens_estimate.input_tokens * input_cost_per_token + 
    ...
    def recommend_models(self, 
                        ...
                        optimization_strategy: str = "balanced") -> List[ModelRecommendation]:
        """推荐最佳模型"""
        ...
        for channel in available_channels:
            ...
            # 计算预估响应时间
            estimated_time = tokens_estimate.estimated_output_tokens / max(speed_score, 10)
            ...
            if tokens_estimate.task_complexity == TaskComplexity.EXPERT and quality_score > 0.80:
        ...
        if optimization_strategy == "cost_first":
            ...
            for rec in recommendations:
                ...
                quality_score = 1.0 - rec.quality_score            # 质量归一化（越高越好转换为越低越好）
                
                rec.combined_score = cost_score * 0.4 + time_score * 0.3 + quality_score * 0.3
