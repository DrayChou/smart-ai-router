...
class AuthenticationError(SmartRouterException):
    """认证相关错误"""
    def __init__(self, message: str, context: Optional[dict] = None, cause: Optional[Exception] = None):
        super().__init__(message, ErrorCategory.AUTH, ErrorSeverity.HIGH, context, cause)


class ConfigurationError(SmartRouterException):
    """配置相关错误"""
    def __init__(self, message: str, context: Optional[dict] = None, cause: Optional[Exception] = None):
        super().__init__(message, ErrorCategory.CONFIG, ErrorSeverity.HIGH, context, cause)


class ExternalAPIError(SmartRouterException):
    """外部API调用错误"""
    def __init__(self, message: str, context: Optional[dict] = None, cause: Optional[Exception] = None):
        super().__init__(message, ErrorCategory.EXTERNAL_API, ErrorSeverity.MEDIUM, context, cause)
    ...
    def decorator(func: Callable):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                # 处理自定义异常
                _log_smart_router_exception(e, operation_name, log_level, include_traceback)
...
def validate_not_empty(value: Union[str, list, dict], name: str, context: Optional[dict] = None) -> Any:
    """验证值不为空"""
    if not value:
        raise ValidationError(
            f"Required parameter '{name}' cannot be empty",
            context=context
        )
    return value
