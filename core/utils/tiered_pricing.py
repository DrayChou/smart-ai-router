...
class TieredPricingCalculator:
    ...
    def get_model_pricing(self, 
                         ...
                         input_tokens: int, 
                         output_tokens: int = 0,
                         use_batch: bool = False) -> Optional[PricingResult]:
        """
        获取模型定价
    ...
    def _calculate_fixed_pricing(self, model_config: Dict[str, Any], use_batch: bool) -> PricingResult:
        """计算固定定价"""
        pricing_key = "batch_pricing" if use_batch else "online_pricing"
        pricing_config = model_config.get(pricing_key, model_config.get("online_pricing", {}))
    ...
    def _calculate_tiered_pricing(self, 
                                model_config: Dict[str, Any], 
                                input_tokens: int, 
                                use_batch: bool) -> Optional[PricingResult]:
        """计算阶梯定价（基于输入长度）"""
    ...
    def _calculate_complex_tiered_pricing(self, 
                                        model_config: Dict[str, Any], 
                                        input_tokens: int, 
                                        output_tokens: int,
                                        use_batch: bool) -> Optional[PricingResult]:
        ...
        logger.warning(f"未找到匹配的复杂价格档位，输入: {input_tokens}, 输出: {output_tokens}")
        return None
    
    def _matches_complex_condition(self, tier: Dict[str, Any], input_tokens: int, output_tokens: int) -> bool:
        """检查是否匹配复杂条件"""
        # 检查输入长度条件
        input_min = tier.get("input_min", 0)
        input_max = tier.get("input_max")
    ...
    def _create_pricing_result_from_tier(self, tier: Dict[str, Any], use_batch: bool) -> PricingResult:
        """从价格档位创建定价结果"""
        pricing_key = "batch_pricing" if use_batch else "online_pricing"
    ...
    def calculate_cost(self, 
                      ...
                      input_tokens: int, 
                      output_tokens: int,
                      use_batch: bool = False) -> Optional[Tuple[float, str]]:
        """
        计算总成本
        ...
        Returns:
            (总成本, 档位信息) 或 None
        """
        pricing_result = self.get_model_pricing(model_name, input_tokens, output_tokens, use_batch)
...
def calculate_doubao_cost(model_name: str, 
                         input_tokens: int, 
                         output_tokens: int = 0,
                         use_batch: bool = False) -> Optional[Tuple[float, str]]:
    """便捷函数：计算豆包模型成本"""
...
if __name__ == "__main__":
    ...
    for input_tokens, output_tokens in test_cases:
        cost, info = calculator.calculate_cost("doubao-seed-1.6-vision", input_tokens, output_tokens)
        print(f"doubao-seed-1.6-vision ({input_tokens//1000}k输入): {cost:.6f}元 ({info})")
    ...
    for input_tokens, output_tokens in complex_cases:
        cost, info = calculator.calculate_cost("doubao-seed-1.6", input_tokens, output_tokens)
        print(f"doubao-seed-1.6 ({input_tokens//1000}k输入, {output_tokens}输出): {cost:.6f}元 ({info})")