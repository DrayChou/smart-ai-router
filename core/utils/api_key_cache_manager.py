...
from .model_analyzer import get_model_analyzer, ModelSpecs
...
class ApiKeyCacheManager:
    ...
    def save_api_key_models(self, 
        ...
        if 'models' in models_data:
            ...
            for model_name in model_list:
                ...
                analyzed_models[model_name] = {
                    ...
                    'pricing': self._extract_pricing_info(model_detail) if model_detail else None
                }
        ...
        cache_data = {
            ...
            'basic_info': {
                ...
                'last_updated': models_data.get('last_updated', datetime.now().isoformat()),
            ...
            'analysis_metadata': {
                ...
                'models_with_params': sum(1 for m in analyzed_models.values() if m['parameter_count']),
                'models_with_context': sum(1 for m in analyzed_models.values() if m['context_length']),
                'models_with_pricing': sum(1 for m in analyzed_models.values() if m.get('pricing'))
            }
        }
        ...
        try:
            ...
            logger.info(f"✅ Saved API key level cache for {channel_id} (key: {api_key_hash[:8]}...): {len(analyzed_models)} models")
            logger.info(f"   📊 Models with parameter info: {cache_data['analysis_metadata']['models_with_params']}")
            logger.info(f"   💰 Models with pricing info: {cache_data['analysis_metadata']['models_with_pricing']}")
            
        except Exception as e:
            logger.error(f"❌ Failed to save API key cache for {channel_id}: {e}")
    
    def _extract_pricing_info(self, model_detail: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """从模型详情中提取定价信息"""
        if not model_detail:
            return None
        ...
        for field in model_detail:
            if any(keyword in field.lower() for keyword in ['input', 'prompt', 'completion', 'output']):
                if any(price_keyword in field.lower() for price_keyword in ['price', 'cost', 'rate']):
    ...
    def load_api_key_models(self, channel_id: str, api_key: str) -> Optional[Dict[str, Any]]:
        """加载特定API Key的模型数据"""
        cache_key = self._get_cache_key(channel_id, api_key)
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        valid_memory_entries = sum(1 for key in self._memory_cache if self._is_cache_valid(key))
