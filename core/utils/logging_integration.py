...
from .smart_logging import SmartLogFilter, setup_smart_logging, get_smart_logger
        ...
        for logger_name, logger in self.enhanced_loggers.items():
            for handler in logger.handlers:
                ...
                handler.filters = [
                    ...
                    if not isinstance(f, SmartLogFilter)
                ]
...
def log_api_response(logger: logging.Logger, status_code: int, 
                    response_time: float, body: Optional[str] = None,
                    **context) -> None:
    """
    记录API响应的便捷函数
...
def log_channel_operation(logger: logging.Logger, operation: str, 
                         channel_id: str, model: str = None, 
...
def log_cost_info(logger: logging.Logger, cost: float, model: str,
                 channel_id: str, tokens: Dict[str, int] = None,
                 **context) -> None:
    """
    记录成本信息的便捷函数
