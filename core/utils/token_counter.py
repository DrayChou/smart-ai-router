...
class TokenCounter:
    ...
    def estimate_completion_tokens(
        cls, prompt_tokens: int, max_tokens: Optional[int] = None
    ) -> int:
        """估算完成token数量"""
        if max_tokens:
            # 使用用户指定的max_tokens
    ...
    def get_token_stats(
        ...
        return {
            ...
            "encoding_method": (
                "tiktoken" if cls._tiktoken_encoder != "simple" else "simple"
            ),
        }
...
class CostTracker:
    ...
    def get_session_summary(self) -> dict[str, Any]:
        ...
        total_tokens = sum(
            record["tokens"].get("total_tokens", 0) for record in self.session_costs
        )
        ...
        return {
            ...
            "cost_per_1k_tokens": (
                (self.total_cost / total_tokens * 1000) if total_tokens > 0 else 0.0
            ),
    ...
    def get_cost_by_channel(self) -> dict[str, float]:
        ...
        for record in self.session_costs:
            ...
            channel_costs[channel_id] = (
                channel_costs.get(channel_id, 0.0) + record["cost"]
            )
