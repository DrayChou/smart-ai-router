...
class AsyncFileManager:
    ...
    async def read_json(self, file_path: Union[str, Path], default: Any = None) -> Any:
        ...
        try:
            ...
            logger.debug(f"📖 ASYNC READ: {path.name} ({len(content)} bytes, {read_time:.1f}ms)")
    ...
    async def write_json(self, file_path: Union[str, Path], data: Any, 
        ...
        try:
            ...
            async with self._get_file_lock(path):
                ...
                logger.debug(f"💾 ASYNC WRITE: {path.name} ({len(json_content)} bytes, {write_time:.1f}ms)")
    ...
    async def read_yaml(self, file_path: Union[str, Path], default: Any = None) -> Any:
        ...
        try:
            ...
            read_time = (time.time() - start_time) * 1000
            logger.debug(f"📖 ASYNC READ: {path.name} YAML ({len(content)} bytes, {read_time:.1f}ms)")
    ...
    async def write_yaml(self, file_path: Union[str, Path], data: Any, 
        ...
        try:
            ...
            async with self._get_file_lock(path):
                ...
                write_time = (time.time() - start_time) * 1000
                logger.debug(f"💾 ASYNC WRITE: {path.name} YAML ({len(yaml_content)} bytes, {write_time:.1f}ms)")
    ...
    async def read_text(self, file_path: Union[str, Path], encoding: str = 'utf-8') -> Optional[str]:
        """异步读取文本文件"""
        try:
            path = Path(file_path)
            if not await aiofiles.os.path.exists(path):
            ...
            read_time = (time.time() - start_time) * 1000
            logger.debug(f"📖 ASYNC READ: {path.name} ({len(content)} chars, {read_time:.1f}ms)")
    ...
    async def write_text(self, file_path: Union[str, Path], content: str, 
                        encoding: str = 'utf-8', ensure_dir: bool = True) -> bool:
        """异步写入文本文件"""
        try:
            path = Path(file_path)
            ...
            async with self._get_file_lock(path):
                ...
                write_time = (time.time() - start_time) * 1000
                logger.debug(f"💾 ASYNC WRITE: {path.name} ({len(content)} chars, {write_time:.1f}ms)")
    ...
    async def append_text(self, file_path: Union[str, Path], content: str, 
                         encoding: str = 'utf-8', ensure_dir: bool = True) -> bool:
        """异步追加文本到文件"""
        try:
            path = Path(file_path)
            ...
            async with self._get_file_lock(path):
                ...
                write_time = (time.time() - start_time) * 1000
                logger.debug(f"➕ ASYNC APPEND: {path.name} (+{len(content)} chars, {write_time:.1f}ms)")
    ...
    async def backup_file(self, file_path: Union[str, Path], 
    ...
    async def batch_read_json(self, file_paths: List[Union[str, Path]], 
    ...
    async def batch_write_json(self, file_data: Dict[Union[str, Path], Any], 
...
async def async_read_text(file_path: Union[str, Path], encoding: str = 'utf-8') -> Optional[str]:
    """便捷的异步文本读取函数"""
