...
class ChannelCacheManager:
    ...
    def save_channel_models(self, channel_id: str, models_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.info(f"✅ Saved channel cache for {channel_id}: {len(analyzed_models)} models analyzed")
            logger.info(f"   📊 Models with parameter info: {cache_data['analysis_metadata']['models_with_params']}")
            logger.info(f"   📏 Models with context info: {cache_data['analysis_metadata']['models_with_context']}")
    ...
    def migrate_from_old_cache(self, old_cache_file: str = "cache/discovered_models.json") -> None:
        """从旧的合并缓存迁移到新的分离缓存"""
    ...
    def search_models_by_specs(self, 
                              min_params: Optional[int] = None,
                              min_context: Optional[int] = None,
                              channel_ids: Optional[List[str]] = None) -> List[Dict[str, Any]]:
        """根据规格搜索模型"""
        results = []
