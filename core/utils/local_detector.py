...
from ..models.model_info import ModelInfo, ModelCapabilities, ModelSpecs, ModelPricing, DataSource
...
class LocalModelDetector:
    ...
    async def detect_capabilities(self, 
        ...
        function_task = self._test_function_calling_capability(base_url, api_key, model_name)
        ...
        logger.info(f"本地模型检测完成: {model_name} - Vision: {model_info.capabilities.supports_vision}, Function: {model_info.capabilities.supports_function_calling}")
    ...
    def _apply_basic_local_inference(self, model_info: ModelInfo) -> None:
        ...
        model_info.capabilities.supports_vision = any(vm in model_lower for vm in vision_models)
        
        # 基于名称推断函数调用能力
        function_models = ["hermes", "mixtral", "qwen"]
        if provider_lower == "ollama":
            # Ollama大部分模型不支持函数调用
            model_info.capabilities.supports_function_calling = any(fm in model_lower for fm in function_models)
        else:
            # 其他本地提供商可能支持
