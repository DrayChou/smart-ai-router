...
class UsageTracker:
    ...
    def record_usage(self, record: UsageRecord):
        ...
        try:
            with self._write_lock:
                ...
                logger.debug(f"记录使用情况: {record.request_id}, 成本: ${record.total_cost:.6f}")
    ...
    def get_daily_stats(self, target_date: Optional[date] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if stats['total_tokens'] > 0:
                stats['avg_cost_per_1k_tokens'] = (stats['total_cost'] / stats['total_tokens']) * 1000
    ...
    def _merge_stats(self, target_stats: Dict[str, Any], source_stats: Dict[str, Any]):
        ...
        for category in ['providers', 'channels', 'models']:
            for key, value in source_stats[category].items():
                if key not in target_stats[category]:
                    target_stats[category][key] = {'requests': 0, 'cost': 0.0, 'tokens': 0}
        ...
        if target_stats['total_requests'] > 0:
            target_stats['avg_cost_per_request'] = target_stats['total_cost'] / target_stats['total_requests']
        if target_stats['total_tokens'] > 0:
            target_stats['avg_cost_per_1k_tokens'] = (target_stats['total_cost'] / target_stats['total_tokens']) * 1000
    ...
    def archive_old_logs(self, days_to_keep: int = 30):
        ...
        try:
            ...
            for log_file in self.logs_dir.glob("usage_*.jsonl"):
                try:
                    # 从文件名解析日期
                    date_str = log_file.stem.split('_')[1]  # usage_20250824.jsonl -> 20250824
