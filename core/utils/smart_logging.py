...
class SmartLogFilter(logging.Filter):
    ...
    def __init__(self, enable_sensitive_cleaning: bool = True, 
    ...
    def _compile_patterns(self):
        ...
        self.api_key_patterns = [
            ...
            re.compile(r'api[_-]?key["\s]*[:=]["\s]*[A-Za-z0-9+/]{10,}', re.IGNORECASE),  # Generic API keys
        ]
        ...
        self.http_body_pattern = re.compile(
            r'"(request|response)_body":\s*"[^"]{100,}"',
            re.IGNORECASE
        )
    ...
    def _simplify_tracebacks(self, message: str) -> str:
        ...
        def simplify_traceback(match):
            ...
            for line in reversed(lines):
                line = line.strip()
                if line and ':' in line and not line.startswith('File ') and not line.startswith('Traceback'):
...
class StructuredFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        # 添加额外的上下文信息
        extra_fields = [
            'source', 'model', 'api_type', 'error_type', 
            'execution_time', 'channel_id', 'request_id',
            'cost', 'tokens', 'cleaned_reduction'
        ]
...
def setup_smart_logging(
    ...
    if console_format == 'simple':
        console_formatter = ColoredFormatter('%(formatted_time)s [%(levelname_colored)s] %(message)s')
    ...
    # 减少第三方库的日志噪声
    noisy_loggers = [
        'httpx', 'openai', 'anthropic', 'uvicorn', 'uvicorn.access',
        'fastapi', 'httpcore', 'urllib3', 'requests'
    ]
...
def create_context_logger(base_logger: logging.Logger, **context) -> logging.LoggerAdapter:
    """
    创建带上下文信息的日志适配器
