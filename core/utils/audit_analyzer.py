...
class AuditAnalyzer:
    ...
    async def get_audit_events(
        ...
        for entry in log_entries:
            # 检查多个可能的位置
            audit_data = entry.get('audit_event') or entry.get('extra_data', {}).get('audit_event')
            if audit_data:
                # 过滤事件类型
    ...
    async def generate_audit_summary(
        ...
        for event in events:
            ...
            # 统计安全事件
            if any(sec_type in event_type for sec_type in [
                'security', 'login.failure', 'rate_limit', 'violation'
            ]):
                security_events += 1
    ...
    async def generate_user_activity_report(
        ...
        peak_activity_times = [f"{hour:02d}:00-{hour+1:02d}:00" for hour, _ in peak_hours]
    ...
    async def detect_anomalies(
        ...
        for user_id, user_events in user_activities.items():
            ...
            login_failures = [
                ...
                if 'login.failure' in e.get('event_type', '')
            ]
            ...
            api_requests = [
                e for e in user_events 
                if e.get('event_type') == 'api.request'
            ]
        ...
        for ip_address, ip_events in ip_activities.items():
            ...
            if len(ip_events) > 500:  # 单IP超过500次请求
                unique_users = len(set(
                    e.get('user_id') for e in ip_events 
                    if e.get('user_id')
                ))
