...
class ChannelMonitor:
    ...
    def record_channel_error(self, channel_id: str, channel_name: str, error_type: str, error_message: str, details: Optional[Dict] = None):
        """记录渠道错误"""
        ...
        if self._should_alert(channel_id, error_type):
            self._send_alert(channel_id, channel_name, error_type, error_message, details)
    
    def record_quota_exhausted(self, channel_id: str, channel_name: str, details: Optional[Dict] = None):
        """记录配额用完"""
        message = f"渠道 {channel_name} 配额已用完"
        self._send_alert(channel_id, channel_name, "quota_exhausted", message, details)
    
    def record_low_balance(self, channel_id: str, channel_name: str, remaining_balance: float, details: Optional[Dict] = None):
        """记录余额不足"""
        ...
        self._send_alert(channel_id, channel_name, "low_balance", message, alert_details)
    
    def record_api_key_invalid(self, channel_id: str, channel_name: str, details: Optional[Dict] = None):
        """记录API密钥无效"""
    ...
    def _should_alert(self, channel_id: str, error_type: str) -> bool:
        ...
        return error_count >= self.max_errors_before_alert and not self._is_in_cooldown(channel_id)
    ...
    def _send_alert(self, channel_id: str, channel_name: str, alert_type: str, message: str, details: Optional[Dict] = None):
        """发送告警"""
        try:
            alert = ChannelAlert(
...
def check_api_error_and_alert(channel_id: str, channel_name: str, status_code: int, error_message: str):
    """检查API错误并发送相应告警"""
    monitor = get_channel_monitor()
    
    if status_code == 401:
        # API密钥无效
        monitor.record_api_key_invalid(channel_id, channel_name, {
        ...
        if 'quota' in error_message.lower() or 'balance' in error_message.lower():
            monitor.record_quota_exhausted(channel_id, channel_name, {
                'status_code': status_code,
                'error_message': error_message
            })
        else:
            monitor.record_channel_error(channel_id, channel_name, 'rate_limit', error_message, {
                'status_code': status_code
            })
    elif status_code >= 500:
        # 服务器错误
        monitor.record_channel_error(channel_id, channel_name, 'server_error', error_message, {
            'status_code': status_code
        })
    else:
        # 其他错误
        monitor.record_channel_error(channel_id, channel_name, 'api_error', error_message, {
            'status_code': status_code
        })