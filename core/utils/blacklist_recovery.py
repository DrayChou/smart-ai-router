...
from .model_channel_blacklist import get_model_blacklist_manager, ModelChannelBlacklistEntry, ErrorType
...
class BlacklistRecoveryManager:
    ...
    async def _perform_recovery_check(self):
        ...
        if success_count > 0:
            logger.info(f"✅ Recovery completed: {success_count} successful, {failed_count} failed")
        else:
            logger.debug(f"🔄 Recovery completed: 0 successful, {failed_count} failed")
    
    def _find_recovery_candidates(self) -> List[Tuple[str, str, ModelChannelBlacklistEntry]]:
        """找到恢复候选项"""
        candidates = []
        ...
        for entry in self.blacklist_manager._blacklist.values():
            ...
            recent_attempts = [
                ...
                if (attempt.channel_id == entry.channel_id and
                    attempt.model_name == entry.model_name and
                    current_time - attempt.attempted_at < timedelta(hours=1))
            ]
    ...
    def _should_attempt_recovery(self, entry: ModelChannelBlacklistEntry, recent_attempts: List[RecoveryAttempt]) -> bool:
        """判断是否应该尝试恢复"""
        # 认证错误通常不会自动恢复
    ...
    async def _attempt_recovery(self, channel_id: str, model_name: str, entry: ModelChannelBlacklistEntry) -> bool:
        """尝试恢复特定的模型-渠道组合"""
        ...
        try:
            ...
            # 执行健康检查
            success, response_time, error_msg = await self._health_check_model(channel, model_name)
            ...
            if success:
                # 恢复成功，从黑名单中移除
                await self.blacklist_manager.remove_blacklist_entry(channel_id, model_name)
                logger.info(f"✅ RECOVERY SUCCESS: {model_name}@{channel_id} is now available (response_time: {response_time:.3f}s)")
                return True
            else:
                # 恢复失败，延长黑名单时间
                logger.debug(f"🔄 Recovery failed for {model_name}@{channel_id}: {error_msg}")
                await self._extend_blacklist(entry, len([a for a in self.recovery_history 
                                                       if a.channel_id == channel_id and a.model_name == model_name and not a.success]))
                return False
                
        except Exception as e:
            logger.error(f"🔄 Recovery attempt error for {model_name}@{channel_id}: {e}")
            return False
    
    async def _health_check_model(self, channel: Channel, model_name: str) -> Tuple[bool, Optional[float], Optional[str]]:
        """对特定模型执行健康检查"""
        start_time = datetime.now()
            ...
            async with httpx.AsyncClient(timeout=self.health_check_timeout) as client:
                ...
                if response.status_code == 200:
                    ...
                    try:
                        ...
                        # 检查模型是否在列表中（支持模糊匹配）
                        model_available = any(
                            model_name.lower() in available_model.lower() or
                            available_model.lower() in model_name.lower()
                        ...
                        if model_available:
                            ...
                            return False, response_time, f"Model {model_name} not found in available models"
    ...
    async def _extend_blacklist(self, entry: ModelChannelBlacklistEntry, failed_attempts: int):
        ...
        logger.debug(f"🔄 Extended blacklist for {entry.model_name}@{entry.channel_id} by {extended_duration}s")
    ...
    def get_recovery_stats(self) -> Dict[str, any]:
        ...
        return {
            ...
            "success_rate": len(successful) / len(recent_attempts) if recent_attempts else 0,
            "avg_response_time": sum(a.response_time for a in successful if a.response_time) / len(successful) if successful else 0,
