...
class ResponseAggregator:
    ...
    def update_tokens(self, request_id: str, prompt_tokens: int, completion_tokens: int, total_tokens: int):
        """更新token信息"""
    ...
    def update_cost(self, request_id: str, request_cost: float, session_cost: float, session_requests: int):
        """更新成本信息"""
        if request_id in self.active_requests:
            metadata = self.active_requests[request_id]
    ...
    def get_final_summary(self, metadata: RequestMetadata) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "performance": {
                "latency_ms": round(metadata.latency * 1000, 2) if metadata.latency else None,
                "ttfb_ms": round(metadata.ttfb * 1000, 2) if metadata.ttfb else None,
                "tokens_per_second": round(metadata.tokens_per_second, 2) if metadata.tokens_per_second else None
            },
            "usage": {
    ...
    def enhance_response_with_summary(self, response_data: Dict[str, Any], metadata: RequestMetadata) -> Dict[str, Any]:
        """为响应数据添加汇总信息（非流式请求使用）"""
