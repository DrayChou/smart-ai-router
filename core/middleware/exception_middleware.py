...
class ExceptionHandlerMiddleware(BaseHTTPMiddleware):
    ...
    async def _handle_exception(self, request: Request, exc: Exception, duration_ms: float) -> JSONResponse:
        """处理异常并返回统一格式的错误响应"""
    ...
    def _create_error_response(self, exc: SmartRouterException, request: Request, duration_ms: float) -> Dict[str, Any]:
        """创建统一的错误响应格式"""
        return {
            "error": {
    ...
    def _log_error(self, request: Request, exc: Exception, status_code: int, duration_ms: float):
        """记录错误日志"""
        logger.error(
    ...
    async def _log_to_status_monitor(self, request: Request, status_code: int, duration_ms: float, exc: Exception):
        """记录到状态监控"""
        try:
            ...
            elif request.method == "POST" and request.url.path.endswith("/chat/completions"):
