...
from ..utils.response_aggregator import RequestMetadata, get_response_aggregator
...
from ..utils.usage_tracker import create_usage_record, get_usage_tracker
...
class ChatCompletionHandler:
    ...
    async def handle_request(
        ...
        logger.info(
            f"REQUEST DETAILS: [{request_id}] {len(request.messages)} messages, max_tokens: {request.max_tokens}, temperature: {request.temperature}"
        )

        try:
            # 步骤1: 路由请求获取候选渠道
            routing_result = await self._route_request_with_fallback(
                request, start_time
            )
            if not routing_result.candidates:
                return self._create_no_channels_error(
                    request.model, time.time() - start_time
                )

            # 步骤2: 请求前成本估算和优化建议
            cost_preview = await self._perform_cost_estimation(
                request, routing_result.candidates, request_id
            )

            # 步骤3: 执行请求并处理重试
            return await self._execute_request_with_retry(
                request, routing_result, start_time, request_id, cost_preview
            )

        except TagNotFoundError as e:
            return self._handle_tag_not_found_error(
    ...
    async def handle_stream_request(
        ...
        logger.info(
            f"ANTHROPIC STREAM: [{request_id}] Starting stream request for model '{request.model}'"
        )
        ...
        try:
            ...
            if not routing_result.candidates:
                ...
                return StreamingResponse(
                    self._error_stream_generator(
                        "no_channels_available", "No available channels"
                    ),
            ...
            # 步骤2: 执行流式请求
            return StreamingResponse(
                self._execute_stream_request_with_retry(
                    request, routing_result, start_time, request_id
                ),
                media_type="text/event-stream",
            )
    ...
    async def _execute_stream_request_with_retry(
        self,
        ...
        for attempt_num, routing_score in enumerate(routing_result.candidates, 1):
            ...
            logger.info(
                f"🔍 STREAM INTERVAL CHECK: Channel '{channel.name}' min_interval={min_interval}s"
            )
            if min_interval > 0:
                is_ready = interval_manager.is_channel_ready(channel.id, min_interval)
                logger.info(
                    f"🔍 STREAM INTERVAL CHECK: Channel '{channel.name}' is_ready={is_ready}"
                )
                if not is_ready:
                    wait_time = interval_manager.get_remaining_wait_time(
                        channel.id, min_interval
                    )
                    logger.info(
                        f"⏭️ STREAM SKIP #{attempt_num}: Channel '{channel.name}' needs to wait {wait_time:.1f}s (min_interval={min_interval}s), trying next channel"
                    )
                    continue
            ...
            if min_interval > 0:
                ...
                logger.info(
                    f"🔍 STREAM INTERVAL RECORDED: Channel '{channel.name}' request time recorded"
                )

            try:
                channel_info = self._prepare_channel_request_info(
                    channel, provider, request, routing_score.matched_model
                )
                ...
                metadata = aggregator.create_request_metadata(
                    ...
                    routing_strategy=getattr(
                        self.router, "current_strategy", "balanced"
                    ),
                ...
                async for chunk in self._stream_channel_api_with_summary(
                    channel_info.url,
                    channel_info.headers,
                    channel_info.request_data,
        ...
        yield self._create_stream_error(
            "all_channels_failed", str(last_error or "All channels failed")
        )
    ...
    async def _route_request_with_fallback(
        ...
        logger.info(
            f"CHANNEL ROUTING: Starting routing process for model '{request.model}'"
        )
        ...
        logger.info(
            f"CHANNEL SELECTION: Processing {len(candidate_channels)} channels with intelligent routing"
        )
        ...
        try:
            ...
            for channel_score in candidate_channels[:15]:  # 分析前15个候选
                ...
                if input_price == 0.0 and output_price == 0.0:
                    pricing_info = self._get_model_pricing(
                        channel.model_name, channel.provider
                    )
                    if pricing_info:
            ...
            logger.info(
                f"🧠 TOKEN ESTIMATE: [{request_id}] Input: {token_estimate.input_tokens}, "
                f"Output: {token_estimate.estimated_output_tokens} (total: {token_estimate.total_tokens})"
            )
            logger.info(
                f"🧠 TASK COMPLEXITY: [{request_id}] {complexity_name.upper()} "
            ...
            if recommendations:
                ...
                if free_options:
                    logger.info(
                        f"💰 FREE OPTIONS: [{request_id}] Found {len(free_options)} free channels"
                    )
                ...
                if len(costs) > 1:
                    ...
                    if savings > 0.001:  # 只有当节省超过0.001美元时才提示
                        logger.info(
                            f"💰 COST SAVINGS: [{request_id}] Can save up to ${savings:.6f} by choosing optimal model"
                        )
    ...
    async def _perform_concurrent_channel_check(
        ...
        """执行并发渠道可用性检查"""
        logger.info(
            f"FAST CHECK: Pre-checking availability of top {min(3, len(candidate_channels))} channels"
        )
        ...
        for i, routing_score in enumerate(candidate_channels[:3]):
            ...
            if provider:
                channel_info = self._prepare_channel_request_info(
                    channel, provider, None, routing_score.matched_model
                )
                check_tasks.append(
                    (
                        i,
                        channel,
                        self._fast_channel_check(
                            channel_info.url, channel_info.headers
                        ),
                    )
                )
        ...
        if check_tasks:
            check_results = await asyncio.gather(
                *[task[2] for task in check_tasks], return_exceptions=True
            )
            ...
            for i, (original_index, channel, _result) in enumerate(check_tasks):
                if i < len(check_results) and not isinstance(
                    ...
                    if is_available:
                        ...
                        min_interval = getattr(channel, "min_request_interval", 0)
                        logger.info(
                            f"FAST CHECK DEBUG: Channel '{channel.name}' min_interval={min_interval} (id={channel.id}) type={type(channel)}"
                        )
                        if min_interval > 0:
                            is_ready = interval_manager.is_channel_ready(
                                channel.id, min_interval
                            )
                            if not is_ready:
                                wait_time = interval_manager.get_remaining_wait_time(
                                ...
                                logger.info(
                                    f"🔍 FAST CHECK SKIP: Channel '{channel.name}' needs to wait {wait_time:.1f}s (min_interval={min_interval}s), not prioritizing"
                                )
                                continue

                        logger.info(
                            f"FAST CHECK: Channel '{channel.name}' is available (HTTP {status_code}), prioritizing"
                        )
                        ...
                        logger.info(
                            f"FAST CHECK: Channel '{channel.name}' unavailable ({status_code}: {message})"
                        )
    ...
    async def _execute_request_with_retry(
        ...
        for attempt_num, routing_score in enumerate(routing_result.candidates, 1):
            ...
            if channel.id in failed_channels:
                logger.info(
                    f"⚫ SKIP #{attempt_num}: Channel '{channel.name}' (ID: {channel.id}) is blacklisted due to previous failures"
                )
                continue

            if not provider:
                logger.warning(
                    f"ATTEMPT #{attempt_num}: Provider '{channel.provider}' for channel '{channel.name}' not found, skipping"
                )
                continue
            ...
            min_interval = getattr(channel, "min_request_interval", 0)
            logger.info(
                f"🔍 INTERVAL CHECK: [{request_id}] Channel '{channel.name}' min_interval={min_interval}s"
            )
            if min_interval > 0:
                is_ready = interval_manager.is_channel_ready(channel.id, min_interval)
                logger.info(
                    f"🔍 INTERVAL CHECK: [{request_id}] Channel '{channel.name}' is_ready={is_ready}"
                )
                if not is_ready:
                    wait_time = interval_manager.get_remaining_wait_time(
                        channel.id, min_interval
                    ...
                    logger.info(
                        f"⏭️ SKIP #{attempt_num}: Channel '{channel.name}' needs to wait {wait_time:.1f}s (min_interval={min_interval}s), trying next channel"
                    )
                    continue

            logger.info(
                f"ATTEMPT #{attempt_num}: [{request_id}] Trying channel '{channel.name}' (ID: {channel.id}) with score {routing_score.total_score:.3f}"
            )
            logger.info(
                f"ATTEMPT #{attempt_num}: [{request_id}] Score breakdown - {routing_score.reason}"
            )
            ...
            if min_interval > 0:
                interval_manager.record_request(channel.id)
                logger.info(
                    f"🔍 INTERVAL RECORDED: [{request_id}] Channel '{channel.name}' request time recorded"
                )
            ...
            try:
                ...
                logger.info(
                    f"📡 FORWARDING: [{request_id}] Sending request to {channel_info.url}"
                )
                logger.info(
                    f"📡 FORWARDING: [{request_id}] Target model -> '{channel_info.request_data['model']}'"
                )
                ...
                self._handle_request_error(
                    e, channel, attempt_num, routing_result.candidates
                )
                continue

        # 所有渠道都失败了
        return self._create_all_channels_failed_error(
            request.model,
            routing_result.candidates,
            last_error,
    ...
    def _handle_streaming_request(
        self,
        request: ChatCompletionRequest,
        channel_info: ChannelRequestInfo,
        ...
        return StreamingResponse(
            self._stream_channel_api_with_summary(
                channel_info.url,
                channel_info.headers,
                channel_info.request_data,
                channel_info.channel.id,
    ...
    async def _handle_regular_request(
        ...
        """处理常规请求"""
        logger.info(
            f"⏳ REQUEST: [{metadata.request_id}] Sending optimized request to channel '{channel_info.channel.name}'"
        )

        response_json, ttfb = await self._call_channel_api(
            channel_info.url, channel_info.headers, channel_info.request_data
        )
        ...
        # 记录传统日志（保持兼容性）
        logger.info(
            f"SUCCESS: [{metadata.request_id}] Channel '{channel_info.channel.name}' responded successfully (latency: {latency:.3f}s)"
        )
        logger.info(
            f"TIMING: [{metadata.request_id}] TTFB: {ttfb*1000:.1f}ms, Total: {latency*1000:.1f}ms"
        )
        logger.info(
            f"RESPONSE: [{metadata.request_id}] Model used -> {response_json.get('model', 'unknown')}"
        )
        logger.info(
            f"RESPONSE: [{metadata.request_id}] Usage -> {response_json.get('usage', {})}"
        )
        ...
        # 计算成本信息
        cost_info = self._calculate_request_cost(
        ...
        # 更新聚合器 (传递TTFB信息)
        aggregator.update_tokens(
            metadata.request_id, prompt_tokens, completion_tokens, total_tokens
        )
        aggregator.update_cost(
            metadata.request_id,
        ...
        # 使用新的响应汇总格式
        enhanced_response = aggregator.enhance_response_with_summary(
            cleaned_response_json, final_metadata
        )
        ...
        try:
            ...
            if request:
                ...
                if matched_model:
                    ...
                    logger.info(
                        f"📡 MODEL SELECTION: Using matched model '{matched_model}' for routing"
                    )
                elif request.model.startswith("auto:") or request.model.startswith(
                    "tag:"
                ):
                    request_dict["model"] = channel.model_name
                    logger.info(
                        f"📡 MODEL SELECTION: Using channel default model '{channel.model_name}' for virtual query"
                    )
                else:
                    request_dict["model"] = request.model
                    logger.info(
                        f"📡 MODEL SELECTION: Using requested model '{request.model}' for physical query"
                    )
                ...
                if adapter_result.get("adapter"):
                    ...
                    adapter_result["request_data"] = (
                        adapter_manager.enhance_request_for_cost_optimization(
                            adapter_result["request_data"],
    ...
    def _create_debug_headers(
        ...
        headers = {
            ...
            "X-Router-Provider": getattr(
                channel_info.provider, "name", channel_info.channel.provider
            ),
            "X-Router-Model": routing_score.matched_model
            or channel_info.channel.model_name,
    ...
    def _invalidate_model_cache(
        ...
        try:
            cache = get_request_cache()
            # 使该模型-渠道组合的缓存失效
            invalidated_count = cache.invalidate_model_channel_combination(
            ...
            logger.warning(
                f"MODEL CACHE INVALIDATION FAILED for {model_name}@{channel_id}: {e}"
            )
    ...
    async def _handle_http_status_error(
        ...
        logger.warning(
            f"ATTEMPT #{attempt_num} FAILED: Channel '{channel.name}' -> {model_name} returned HTTP {error.response.status_code}"
        )
        logger.warning(f"ERROR DETAILS: {error_text[:200]}...")

        # 记录渠道错误并发送告警
        check_api_error_and_alert(
            channel.id, channel.name, error.response.status_code, error_text
        )
        ...
        if should_blacklist_channel:
            ...
            failed_channels.add(channel.id)
            logger.error(
                f"🔴 CHANNEL BLACKLISTED: Channel '{channel.name}' (ID: {channel.id}) blacklisted due to multiple failures or critical errors"
            )
            self._invalidate_channel_cache(
                channel.id, channel.name, "channel-level failure"
            )
        else:
            # 仅模型级别黑名单
            logger.warning(
                f"🚫 MODEL BLACKLISTED: Model '{model_name}' blacklisted on channel '{channel.name}' due to HTTP {error.response.status_code}"
            )
        ...
        if error.response.status_code == 429:
            ...
            if wait_time:
                logger.warning(
                    f"SMART RATE LIMIT: {model_name}@{channel.name} suggests waiting {wait_time}s, but switching to next channel instead"
                )
            else:
                logger.warning(
                    f"RATE LIMIT: {model_name}@{channel.name} rate limited, switching to next channel immediately"
                )
    ...
    def _handle_request_error(
        ...
        """处理请求错误"""
        logger.warning(
            f"ATTEMPT #{attempt_num} FAILED: Channel '{channel.name}' network error: {str(error)}"
        )
        self.router.update_channel_health(channel.id, False)
    ...
    def _create_no_channels_error(
        self, model: str, execution_time: float
    ) -> JSONResponse:
        """创建无可用渠道错误响应"""
    ...
    def _handle_tag_not_found_error(
        self, error: TagNotFoundError, model: str, execution_time: float
    ) -> JSONResponse:
        """处理标签未找到错误"""
        ...
        return JSONResponse(
            status_code=404, content={"detail": str(error)}, headers=headers
        )

    def _handle_unexpected_error(
        self, error: Exception, model: str, execution_time: float
    ) -> HTTPException:
        """处理意外错误"""
        logger.error(
            f"💥 UNEXPECTED ERROR: Internal error occurred after {execution_time:.3f}s: {error}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=500, detail=f"An internal server error occurred: {error}"
        )
    ...
    def _create_all_channels_failed_error(
        ...
        """创建所有渠道失败错误响应"""
        logger.error(
            f"💥 ALL CHANNELS FAILED: All {len(candidates)} channels failed for model '{model}' after {execution_time:.3f}s"
        )
        ...
        return JSONResponse(
            status_code=503, content={"detail": error_detail}, headers=headers
        )
    ...
    async def _stream_channel_api(
        self, url: str, headers: dict, request_data: dict, channel_id: str
    ):
        """优化的流式API调用"""
        chunk_count = 0
        ...
        logger.info(
            f"STREAM START: Initiating optimized streaming request to channel '{channel_id}'"
        )
        ...
        try:
            ...
            async with http_pool.stream(
                ...
                if response.status_code != 200:
                    ...
                    logger.error(
                        f"STREAM ERROR: Channel '{channel_id}' returned status {response.status_code}"
                    )
                ...
                logger.info(
                    f"STREAM CONNECTED: Successfully connected to channel '{channel_id}', starting optimized data flow"
                )
                ...
                async for chunk in response.aiter_bytes(chunk_size=8192):
                    if chunk:
                        ...
                        if not first_token_received:
                            ...
                            logger.info(
                                f"FIRST TOKEN: Received first token from channel '{channel_id}' in {ttfb:.3f}s"
                            )

                        if chunk_count % 20 == 0:
                            logger.debug(
                                f"STREAMING: Received {chunk_count} chunks from channel '{channel_id}'"
                            )
                    yield chunk
            ...
            logger.info(
                f"STREAM COMPLETE: Channel '{channel_id}' completed optimized streaming {chunk_count} chunks in {stream_duration:.3f}s"
            )

        except httpx.HTTPStatusError as e:
            error_text = e.response.text if hasattr(e.response, "text") else str(e)
            logger.error(
                f"STREAM FAILED: Channel '{channel_id}' HTTP error {e.response.status_code}: {error_text[:200]}..."
            )
            self.router.update_channel_health(channel_id, False)
            ...
            logger.error(
                f"STREAM EXCEPTION: Streaming request for channel '{channel_id}' failed: {e}",
                exc_info=True,
            )
            self.router.update_channel_health(channel_id, False)
    ...
    def _clean_response_content(self, response_json: dict) -> dict:
        ...
        try:
                ...
                if isinstance(cleaned_choice, dict) and "message" in cleaned_choice:
                    ...
                    if isinstance(message, dict) and "content" in message:
                        ...
                        if isinstance(original_content, str) and original_content:
                            ...
                            should_clean_thinking = safe(
                                self.config_loader
                            ).config.clean_thinking_chains.value(True)
            ...
            logger.warning(
                f"Response content cleaning failed: {e}, returning original response"
            )
            return response_json

    def _add_cost_information(
        self, response_json: dict, channel, routing_score: RoutingScore
    ) -> dict:
        """为响应添加实时成本信息"""
        try:
            # 获取usage信息
            ...
            if not usage:
                logger.warning(
                    "No usage information in response, cannot calculate cost"
                )
                return response_json
            ...
            cost_info = self._calculate_request_cost(
                channel, prompt_tokens, completion_tokens, model_used
            )
            ...
            enhanced_usage.update(
                {
                    ...
                    "session_cost": session_summary.get(
            ...
            logger.info(
                f"💰 COST: Request cost ${cost_info['total_cost']:.6f} | Session total: {session_summary.get('formatted_total_cost', '$0.00')}"
            )
    ...
    def _calculate_request_cost(
        ...
        if model_name:
            try:
                ...
                if channel.id in model_cache:
                    ...
                    if (
                        isinstance(discovered_info, dict)
                        and "models_data" in discovered_info
                    ):
                        models_data = discovered_info["models_data"]
                        if model_name in models_data:
                            model_data = models_data[model_name]
                            if (
                                "raw_data" in model_data
                                and "pricing" in model_data["raw_data"]
                            ):
                                ...
                                if prompt_cost == 0.0 and completion_cost == 0.0:
                                    logger.debug(
                                        f"COST: Using model-level free pricing for '{model_name}' (prompt={prompt_cost}, completion={completion_cost})"
                                    )
                                    input_cost_per_token = 0.0
                                    output_cost_per_token = 0.0
                                else:
                                    # 模型有明确的收费定价
                                    logger.debug(
                                        f"COST: Using model-level pricing for '{model_name}' (prompt={prompt_cost}, completion={completion_cost})"
                                    )
                                    input_cost_per_token = prompt_cost
                                    output_cost_per_token = completion_cost
                            else:
                                logger.debug(
                                    f"COST: No model-level pricing found for '{model_name}', falling back to channel pricing"
                                )
                        else:
                            logger.debug(
                                f"COST: Model '{model_name}' not found in cache for channel '{channel.id}'"
                            )
                    else:
                        logger.debug(
                            f"COST: Invalid cache structure for channel '{channel.id}'"
                        )
            except Exception as e:
                logger.warning(
        ...
        if input_cost_per_token == 0.0 and output_cost_per_token == 0.0:
            ...
            if model_name and (
                ...
                logger.debug(
                    f"COST: Model '{model_name}' has explicit :free suffix, using zero cost"
                )
                input_cost_per_token = 0.0
                output_cost_per_token = 0.0
            # 使用渠道级别定价
                ...
                logger.debug(
                    f"COST: Using channel cost_per_token (input={input_cost_per_token}, output={output_cost_per_token})"
                )
            # 回退到pricing配置
            elif hasattr(channel, "pricing") and channel.pricing:
                input_cost_per_token = (
                    channel.pricing.get("input_cost_per_1k", 0.001) / 1000
                )
                output_cost_per_token = (
                    channel.pricing.get("output_cost_per_1k", 0.002) / 1000
                )
                logger.debug(
                    f"COST: Using channel pricing (input={input_cost_per_token}, output={output_cost_per_token})"
                )
            else:
                # 默认估算值
                ...
                logger.debug(
                    f"COST: Using default pricing (input={input_cost_per_token}, output={output_cost_per_token})"
                )
    ...
    async def _stream_channel_api_with_summary(
        self,
        url: str,
        headers: dict,
        request_data: dict,
        ...
        logger.info(
            f"STREAM START: [{metadata.request_id}] Initiating optimized streaming request to channel '{channel_id}'"
        )
        ...
        try:
            ...
            async with http_pool.stream(
                ...
                if response.status_code != 200:
                    ...
                    if len(error_body) > 1024:
                        error_body = error_body[:1024]
                    logger.error(
                        f"STREAM ERROR: [{metadata.request_id}] Channel '{channel_id}' returned status {response.status_code}"
                    )

                    error_text = error_body.decode("utf-8", errors="ignore")
                    logger.error(
                        f"STREAM ERROR DETAILS: [{metadata.request_id}] {error_text[:200]}"
                    )
                    self.router.update_channel_health(channel_id, False)
                    ...
                    if response.status_code == 429:
                        ...
                        if wait_time:
                            logger.warning(
                                f"STREAM RATE LIMIT: [{metadata.request_id}] Channel '{channel_id}' suggests waiting {wait_time}s"
                            )
                            # 在错误响应中包含等待时间信息
                    ...
                    # 设置错误信息并完成请求
                    aggregator.set_error(
                        metadata.request_id, str(response.status_code), error_text[:200]
                    )
                    final_metadata = aggregator.finish_request(metadata.request_id)
                ...
                logger.info(
                    f"STREAM CONNECTED: [{metadata.request_id}] Successfully connected to channel '{channel_id}', starting optimized data flow"
                )
                ...
                async for chunk in response.aiter_bytes(chunk_size=8192):
                    if chunk:
                        ...
                        if not ttfb_recorded:
                            ...
                            ttfb_recorded = True
                            logger.info(
                                f"TTFB: [{metadata.request_id}] First data received in {ttfb:.3f}s"
                            )
                                        ...
                                        try:
                                            ...
                                            if "error" in data:
                                                ...
                                                if (
                                                    ...
                                                    wait_time = self._extract_rate_limit_wait_time(
                                                        error_message
                                                    )
                                                    if wait_time:
                                                        logger.warning(
                                                            f"CONTENT RATE LIMIT: [{metadata.request_id}] Channel '{channel_id}' content suggests waiting {wait_time}s"
                                                        )
                                                        # 修改错误信息以包含等待时间
                                                        error_obj["retry_after"] = (
                                                            wait_time
                                                        )
                                                        ...
                                                        chunk = chunk_str.encode(
                                                            "utf-8"
                                                        )
                                                    else:
                                                        logger.warning(
                                                            f"CONTENT RATE LIMIT: [{metadata.request_id}] Channel '{channel_id}' rate limited in streaming content"
                                                        )
                        ...
                        if chunk_count % 20 == 0:
                            logger.debug(
                                f"STREAMING: [{metadata.request_id}] Received {chunk_count} chunks from channel '{channel_id}'"
                            )
                ...
                if total_tokens > 0:
                    ...
                    # 这里简化处理，实际应该传递原始请求对象
                    session_manager = get_session_manager()
                    user_identifier = session_manager.create_user_identifier(
                        "streaming-user", "streaming-client"
                    )
                    session = session_manager.add_request(
                        user_identifier=user_identifier,
                        cost=cost_info["total_cost"],
                    ...
                    if first_data_time and total_completion_tokens > 0:
                        generation_time = (
                            time.time() - first_data_time
                        )  # 从第一个token到最后一个token的时间
                        if generation_time > 0:
                            tokens_per_second = (
                                total_completion_tokens / generation_time
                            )
                            # 更新准确的token生成速度
                            aggregator.update_performance(
                                metadata.request_id, tokens_per_second=tokens_per_second
                            )
                            logger.info(
                                f"TOKEN SPEED: [{metadata.request_id}] {total_completion_tokens} tokens in {generation_time:.3f}s = {tokens_per_second:.1f} tokens/sec"
                            )
            ...
            self.router.update_channel_health(channel_id, True, stream_duration)
            logger.info(
                f"STREAM COMPLETE: [{metadata.request_id}] Channel '{channel_id}' completed optimized streaming {chunk_count} chunks in {stream_duration:.3f}s"
            )
            ...
            error_text = e.response.text if hasattr(e.response, "text") else str(e)
            logger.error(
                f"STREAM FAILED: [{metadata.request_id}] Channel '{channel_id}' HTTP error {e.response.status_code}: {error_text[:200]}..."
            )
            self.router.update_channel_health(channel_id, False)

            # 设置错误信息并完成请求
            aggregator.set_error(
                metadata.request_id, str(e.response.status_code), error_text
            )
            final_metadata = aggregator.finish_request(metadata.request_id)
            ...
            logger.error(
                f"STREAM EXCEPTION: [{metadata.request_id}] Streaming request for channel '{channel_id}' failed: {e}",
                exc_info=True,
            )
            self.router.update_channel_health(channel_id, False)
