...
class CostOptimizedStrategy(BaseRoutingStrategy):
    ...
    def __init__(self):
        super().__init__(
            name="cost_optimized", description="优先选择成本最低的可用渠道"
        )
    ...
    async def calculate_scores(
        ...
        for i, channel in enumerate(channels):
            ...
            if cost_range > 0:
                cost_score = float(
                    1.0 - float(effective_cost - min_cost) / float(cost_range)
                )
            else:
                cost_score = 1.0
            ...
            score = RoutingScore(
                ...
                speed_score=float(
                    channel.performance_scores.get("speed_score", 0.5)
                    if channel.performance_scores
                    else 0.5
                ),
                quality_score=float(
                    channel.performance_scores.get("quality_score", 0.5)
                    if channel.performance_scores
                    else 0.5
                ),
