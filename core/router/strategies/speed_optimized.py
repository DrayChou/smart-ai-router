...
class SpeedOptimizedStrategy(BaseRoutingStrategy):
    ...
    def __init__(self):
        super().__init__(
            name="speed_optimized", description="优先选择响应速度最快的可用渠道"
        )
    ...
    async def calculate_scores(
        ...
        for channel in channels:
            ...
            cost_factor = (
                1.0 / (1.0 + float(effective_cost)) if effective_cost > 0 else 1.0
            )
            combined_score = (
                0.7 * speed_score + 0.2 * reliability_score + 0.1 * cost_factor
            )
            ...
            score = RoutingScore(
                ...
                metadata={
                    ...
                    "base_speed_score": float(
                        performance_scores.get("speed_score", 0.5)
                    ),
