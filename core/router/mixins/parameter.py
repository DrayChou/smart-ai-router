...
from core.routing.exceptions import ParameterComparisonError
...
class ParameterComparisonMixin:
    ...
    def _get_parameter_comparison_candidates(self, request: RoutingRequest) -> Optional[list[ChannelCandidate]]:
        ...
        if not comparison:
            logger.error("PARAM PARSE FAILED: Could not parse parameter comparison '%s'", request.model)
        ...
        if not matching_models:
            logger.error("PARAM COMPARISON FAILED: No models found matching '%s'", request.model)
            raise ParameterComparisonError(request.model)

        logger.info("📝 First 5 matched models:")
        for index, (channel_id, model_name, model_params) in enumerate(matching_models[:5]):
            logger.info("  %s. %s -> %s (%.3fB)", index + 1, channel_id, model_name, model_params)

        candidates: list[ChannelCandidate] = []
        disabled_count = 0
        not_found_count = 0

        logger.debug("Processing %s matching models for channel lookup...", len(matching_models))
        ...
        for channel_id, model_name, model_params in matching_models:
            ...
            logger.debug("Channel ID mapping: '%s' -> '%s'", channel_id, real_channel_id)

            channel = self.config_loader.get_channel_by_id(real_channel_id)
            if channel:
                if channel.enabled:
                    candidates.append(ChannelCandidate(channel=channel, matched_model=model_name))
                    logger.debug("Added channel: %s -> %s (%.3fB)", channel.name, model_name, model_params)
                else:
                    disabled_count += 1
                    logger.debug("Disabled channel: %s -> %s", real_channel_id, model_name)
            else:
                not_found_count += 1
                logger.debug("Channel not found: %s (from %s) -> %s", real_channel_id, channel_id, model_name)
        ...
        if candidates:
            ...
            for index, candidate in enumerate(candidates[:5]):
                logger.info("  %s. %s -> %s", index + 1, candidate.channel.name, candidate.matched_model)
