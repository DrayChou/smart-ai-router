...
class ConfigLoader:
    ...
    def load_config(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"配置加载成功: {len(self.providers)} providers, {len(self.channels)} channels, {len(self.model_groups)} model groups"
            )
    ...
    def _load_channels(self) -> None:
        ...
        for channel_data in channels_data:
            ...
            if api_key.startswith("${") and api_key.endswith("}"):
                ...
                if not api_key:
                    logger.warning(
                        f"环境变量 {env_var} 未设置，渠道 {channel_data.get('name')} 将被禁用"
                    )
    ...
    def save_config(self) -> None:
        ...
        try:
            ...
            self.config_data["runtime_state"] = {
                ...
                "request_history": self.runtime_state.request_history[
                    -1000:
                ],  # 只保留最近1000条记录
    ...
    def add_request_log(self, log_entry: dict[str, Any]) -> None:
        ...
        if len(self.runtime_state.request_history) > 1000:
            self.runtime_state.request_history = self.runtime_state.request_history[
                -1000:
            ]
