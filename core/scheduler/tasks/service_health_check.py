...
class ServiceHealthChecker:
            ...
            if self.health_cache_file.exists():
                with open(self.health_cache_file, encoding="utf-8") as f:
                    ...
                    self.health_results = {
                        ...
                        for k, v in data.get("health_results", {}).items()
                    }
                    self.provider_health = {
                        k: ProviderHealth(**v)
                        for k, v in data.get("provider_health", {}).items()
                    }
    ...
    def _save_cache(self):
        ...
        try:
            ...
            with open(self.health_history_file, "w", encoding="utf-8") as f:
                json.dump(
                    self.health_history, f, ensure_ascii=False, indent=2, default=str
                )
    ...
    def _select_test_model(
        ...
        for model in available_models:
            ...
            if any(
                keyword in model_id
                for keyword in ["4b", "8b", "12b", "mini", "small", "lite"]
            ):
            ...
            if (
                "free" in model_id
                or "gpt-3.5-turbo" in model_id
                or "claude-3-haiku" in model_id
                or any(
                    keyword in model_id
                    for keyword in ["gemini-pro", "llama-3", "mixtral"]
                )
            ):
    ...
    async def _test_channel_health_via_models(
        ...
        headers = {"User-Agent": "SmartAIRouter/1.0", "Accept": "application/json"}
        ...
        try:
            async with httpx.AsyncClient(timeout=self.http_timeout) as client:
                ...
                if response.status_code == 200:
                    # 成功，渠道健康且顺便更新了模型列表
                    logger.info(
                        f"渠道 {channel_id} 通过/models接口健康检查成功，延迟: {latency_ms:.1f}ms"
                    )
                    ...
                    # /models接口失败，尝试备用方案
                    logger.info(
                        f"渠道 {channel_id} /models接口失败 ({response.status_code})，尝试备用测试方案"
                    )
        ...
        try:
            async with httpx.AsyncClient(timeout=self.http_timeout) as client:
                ...
                if response.status_code in [
                    ...
                    logger.info(
                        f"渠道 {channel_id} 通过虚假模型健康检查成功，延迟: {latency_ms:.1f}ms"
                    )
                    result = HealthCheckResult(
                        channel_id=channel_id,
                        provider=provider,
    ...
    async def check_all_channels(
        ...
        for channel in channels:
            # 跳过没有API密钥的渠道
            if (
                not channel.get("api_key")
                or len(channel.get("api_key", "").strip()) < 10
            ):
                continue
        ...
        results = await asyncio.gather(
            *[limited_check(task) for task in tasks], return_exceptions=True
        )
    ...
    def _calculate_provider_health(self):
        ...
        for provider, stats in provider_stats.items():
            ...
            success_rate = (
                stats["healthy"] / stats["total"] if stats["total"] > 0 else 0
            )
            ...
            self.provider_health[provider] = ProviderHealth(
                ...
                cheapest_healthy_channel=stats[
                    "fastest_channel"
                ],  # 简化：暂时用最快的代替
    ...
    def _log_health_summary(self):
        ...
        for provider, health in self.provider_health.items():
            logger.info(
                f"Provider {provider}: {health.healthy_channels}/{health.total_channels} "
