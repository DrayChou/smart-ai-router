...
class TaskScheduler:
    ...
    def add_task(self, 
        ...
        logger.info(f"已添加任务 '{name}', 间隔 {interval_seconds}s, 立即执行: {run_immediately}")
    ...
    def get_task_status(self) -> Dict[str, Any]:
        ...
        for name, task_info in self.tasks.items():
            status['tasks'][name] = {
                ...
                'success_rate': task_info['success_count'] / max(task_info['run_count'], 1),
...
# 便捷函数
def add_task(name: str, func: Callable, interval_seconds: int, run_immediately: bool = False, **kwargs):
    """添加任务的便捷函数"""
