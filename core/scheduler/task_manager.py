...
class TaskManager:
    ...
    def initialize_tasks(self, config: dict[str, Any]):
        ...
        if model_discovery_config.get("enabled", True):
            interval = (
                model_discovery_config.get("interval_hours", 6) * 3600
            )  # 转换为秒
        ...
        if health_check_config.get("enabled", True):
            ...
            run_immediately = health_check_config.get(
    ...
    async def _run_api_key_validation_task(self):
        ...
        try:
            ...
            logger.info(
                f"API密钥验证完成: {stats.get('valid_keys', 0)}/{stats.get('total_keys', 0)} 密钥有效"
            )
    ...
    async def _run_health_check_task(self):
        ...
        try:
            ...
            stats = result.get("stats", {})
            logger.info(
                f"健康检查完成: {stats.get('healthy_channels', 0)}/{stats.get('total_channels', 0)} 渠道健康"
            )
    ...
    async def _run_cache_cleanup_task(self):
        ...
        try:
            ...
            result = {"success": True, "cleaned_files": cleaned_files}
