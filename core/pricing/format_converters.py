...
from .unified_format import (
    UnifiedModelData, UnifiedPricingFile, Architecture, Pricing, TopProvider,
...
class OpenRouterConverter(BaseFormatConverter):
    ...
    def _convert_single_model(self, model_id: str, model_info: Dict[str, Any]) -> Optional[UnifiedModelData]:
        """转换单个模型"""
        ...
        unified_model = UnifiedModelData(
            ...
            context_length=model_info.get('context_length') or raw_data.get('context_length'),
        ...
        # 推断能力和分类
        unified_model.capabilities = self.capability_inference.infer_capabilities_from_name(model_id)
...
class SiliconFlowConverter(BaseFormatConverter):
    ...
    def convert_to_unified(self, source_data: Dict[str, Any]) -> UnifiedPricingFile:
        ...
        for model_id, model_info in models_data.items():
            try:
                unified_model = self._convert_single_model(model_id, model_info, exchange_rate)
                if unified_model:
                    unified_file.models[model_id] = unified_model
            except Exception as e:
    ...
    def _convert_single_model(self, model_id: str, model_info: Dict[str, Any], exchange_rate: float) -> Optional[UnifiedModelData]:
        """转换单个模型"""
        ...
        if not capabilities:
            capabilities = self.capability_inference.infer_capabilities_from_name(model_id)
...
class FormatConverterFactory:
    ...
    def get_converter(cls, platform: str) -> BaseFormatConverter:
        ...
        if platform not in cls._converters:
            raise ValueError(f"不支持的平台: {platform}, 支持的平台: {list(cls._converters.keys())}")
...
if __name__ == "__main__":
    ...
    if openrouter_file.exists():
        ...
        try:
            unified_file = convert_platform_to_unified(openrouter_file, "openrouter", output_file)
