...
class ChannelException(RouterException):
    ...
    def __init__(self, channel_id: str, message: str, status_code: int = 503, details: Optional[Dict[str, Any]] = None):
...
class AllChannelsFailedException(RouterException):
    ...
    def __init__(self, model: str, attempts: int, last_error: Optional[Exception] = None):
        self.model = model
    ...
    def handle_httpx_error(error: Exception, channel_id: str) -> ChannelException:
        ...
        if isinstance(error, httpx.HTTPStatusError):
            ...
            if status_code in [401, 403]:
                ...
                return ChannelException(channel_id, f"Bad request: {error_text[:200]}", status_code)
            else:
                return ChannelException(channel_id, f"HTTP {status_code}: {error_text[:200]}", status_code)
                
        elif isinstance(error, httpx.RequestError):
            return ChannelUnavailableException(channel_id, f"Network error: {str(error)}")
        else:
            return ChannelException(channel_id, f"Unknown error: {str(error)}")
    
    @staticmethod
    def create_error_response(error: RouterException, execution_time: Optional[float] = None) -> JSONResponse:
        """创建统一的错误响应"""
        headers = {
